...
self.onmessage=async({data})=>{
  ...
  try{
    ...
    const result=createThreats(),total=size===0||size===pool.length?1:96
    ...
    for(let i=0;i<total;i++){
      ...
      if((i+1)%8===0||i===total-1)self.postMessage({id,side:data.side,result,done:i===total-1,exact:total===1})
